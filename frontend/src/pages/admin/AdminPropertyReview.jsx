        ...
        <AdminLayout>
            ...
            {showReject && (
                <div className="bg-red-50 border border-red-200 rounded-2xl p-5 mb-6">
                    <h3 className="font-semibold text-red-800 mb-3">Motivo del rechazo</h3>
                    <textarea
                        value={reason}
                        onChange={e => { setReason(e.target.value); setReasonError('') }}
                        rows={3}
                        placeholder="Explica al usuario por qué se rechaza su anuncio..."
                        className={`w-full px-3 py-2.5 text-sm border rounded-lg outline-none resize-none
                            ${reasonError ? 'border-red-400 bg-red-50' : 'border-red-200 focus:border-red-400 bg-white'}`}
                    />
            ...
            {showDelete && (
                <div className="bg-red-50 border border-red-200 rounded-2xl p-5 mb-6">
                    <h3 className="font-semibold text-red-800 mb-3">Motivo de eliminación</h3>
                    ...
                    <textarea
                        ...
                        className={`w-full px-3 py-2.5 text-sm border rounded-lg outline-none resize-none
                            ${!deleteReason.trim() ? 'border-red-300' : 'border-red-200 focus:border-red-400'} bg-white`}
                    />
                    {!deleteReason.trim() && (
            ...
            <div className="grid grid-cols-3 gap-6">
                ...
                <div className="col-span-2 flex flex-col gap-4">
                    ...
                    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 ">
                        <h2 className="font-semibold text-gray-800 mb-3">Descripción</h2>
                        <p className="text-sm text-gray-600 whitespace-pre-line leading-relaxed wrap-break-word">
                ...
                <div className="flex flex-col gap-4">
                    ...
                    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
                        <p className="text-2xl font-bold text-gray-900">
                            ...
                            {property.operation === 'rent' && <span className="text-base font-normal text-gray-400">/mes</span>}
                        </p>
                    ...
                    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
                        ...
                        <div className="flex flex-col gap-2 text-sm">
                            {property.bedrooms  != null && <div className="flex justify-between"><span className="text-gray-400">Habitaciones</span><span className="font-medium">{property.bedrooms}</span></div>}
                            {property.bathrooms != null && <div className="flex justify-between"><span className="text-gray-400">Baños</span><span className="font-medium">{property.bathrooms}</span></div>}
                            {property.area      != null && <div className="flex justify-between"><span className="text-gray-400">Superficie</span><span className="font-medium">{property.area} m²</span></div>}
                        </div>
                    </div>

                    {/* Extras */}
                    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
                        <h2 className="font-semibold text-gray-800 mb-4">Extras</h2>
                        <div className="flex flex-col divide-y divide-gray-50">
                            {[
                                ...
                                <div key={feat.key} className="flex items-center justify-between py-2.5">
                                    ...
                                    <span className="text-xs font-semibold text-green-700 bg-green-50 px-2 py-0.5 rounded-full">Sí</span>
                                </div>
                            ))}
                            {!property.has_garage && !property.has_elevator && !property.has_pool && !property.has_garden && !property.is_furnished && (
                                <p className="text-sm text-gray-400 py-2">Sin extras</p>
                            )}
                        </div>
                    </div>
                    ...
                    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
                        ...
                        <p className="text-sm text-gray-600 wrap-break-words">{property.address}</p>
                        <p className="text-sm text-gray-400 mt-1 wrap-break-words">{property.city}, {property.province} {property.postal_code}</p>
                    </div>

                    {/* Motivo de rechazo si existe */}
                    {property.rejection_reason && (
                        <div className="bg-red-50 border border-red-200 rounded-2xl p-5">
                            <h2 className="font-semibold text-red-800 mb-2">Motivo de rechazo</h2>
